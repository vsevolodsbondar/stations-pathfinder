...
func TestShouldScheduleTrainsIn6TurnsFor9TrainsAlgorithm1(t *testing.T) {
	config := m.FlagConfig{
		NetworkMapPath:  "./testData/5_test.txt",
		StartingStation: "beethoven",
		EndingStation:   "part",
		TrainNumb:       9,
	}
	...
	if strings.Contains(res, "T1-verdi") {
		contains1 = true
	}

	if strings.Contains(res, "T1-part") {
		contains2 = true
	}

	if !contains1 && !contains2 {
		t.Errorf("Should contain at least T1-verdi and T1-part in output.")
	}
...
func TestShouldScheduleTrainsIn6TurnsFor9TrainsAlgorithm2(t *testing.T) {
	config := m.FlagConfig{
		NetworkMapPath:  "./testData/5_test.txt",
		StartingStation: "beethoven",
		EndingStation:   "part",
	...
	for _, line := range lines {
		if strings.Contains(line, "T1-verdi") {
			contains1 = true
		}

		if strings.Contains(line, "T1-part") {
			contains2 = true
		}
