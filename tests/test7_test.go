...
func TestShouldScheduleTrainsIn6TurnsForAlgorithm1(t *testing.T) {
	config := m.FlagConfig{
		NetworkMapPath:  "./testData/1_test.txt",
		StartingStation: "bond_square",
		EndingStation:   "space_port",
		TrainNumb:       4,
	}
	...
	if strings.Contains(res, "T1-apple_avenue") {
		contains1 = true
	}

	if strings.Contains(res, "T1-space_port") {
		contains2 = true
	}

	if !contains1 && !contains2 {
		t.Errorf("Should contain at least T1-apple_avenue and T1-space_port in output.")
	}
...
func TestShouldScheduleTrainsIn6TurnsForAlgorithm2(t *testing.T) {
	config := m.FlagConfig{
		NetworkMapPath:  "./testData/1_test.txt",
		StartingStation: "bond_square",
		EndingStation:   "space_port",
	...
	for _, line := range lines {
		if strings.Contains(line, "T1-apple_avenue") {
			contains1 = true
		}

		if strings.Contains(line, "T1-space_port") {
			contains2 = true
		}
